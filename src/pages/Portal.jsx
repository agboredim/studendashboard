...
import { Home, Book, HelpCircle, PlayCircle, User, Award, MessageCircle, Bell } from 'lucide-react';
...
const Portal = () => {
  ...
  const notifications = [
    ...
    },
    {
      ...
      message: 'Don’t forget to submit your Chemistry assignment by 5 PM today.',
      time: '1 hour ago',
    },
  ];
  ...
  const CourseProgressView = () => (
    <div className="p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-gray-900">Product Management Course</h2>
      <div className="space-y-4">
        <ProgressBar label="Course Progress" value={65} color="bg-blue-500" />
        <ProgressBar label="Assignment Progress" value={45} color="bg-yellow-400" />
        <ProgressBar label="Discussion Participation" value={80} color="bg-teal-500" />
      </div>
  ...
  const AssessmentView = () => (
    ...
    <div className="flex justify-between items-center mb-1">
      ...
      <span className="text-lg font-bold text-gray-900">{answeredQuestions}</span>
    </div>
    ...
    <div className="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">
      ...
      <ul className="space-y-4">
        {notifications.map((notif) => (
          <li key={notif.id} className="border-l-4 border-blue-600 pl-4 bg-gray-50 p-4 rounded">
  ...
  JOIN LIVE CLASS
    ...
    <img className="w-10 h-10 rounded-full bg-gray-300" src={img} alt="Profile" />
  </div>
</nav>
...
<div className="w-full lg:w-[75%] h-auto bg-transparent pt-8 px-4 sm:px-6 md:px-8">
  {selectedView === 'Dashboard' && (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-2">Welcome Back Joseph</h2>
      <p className="text-base sm:text-lg mb-6">Continue your learning journey with one of your current Courses.</p>
  ...
  {selectedView === 'Dashboard' && (
    <div className="mx-auto max-w-screen-xl px-4 py-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <div>
          ...
          <div className="space-y-4 text-sm">
            {[{ day: 25, title: 'Project Plan Submission' }, { day: 28, title: 'Case Study Analysis' }].map((item, idx) => (
              <div key={idx} className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-md font-medium text-black whitespace-pre-line">{item.title}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-10">
        {[{ title: 'Upcoming Deadlines' }, { title: 'Achievements & Certificates' }].map((section, index) => (
          <div key={index}>
