        ...
        {currentCourses.map((course, index) => (
          <div key={index}>
            ...
            <div className="relative h-3 bg-gray-300 rounded-full">
              <div
                className="absolute top-0 left-0 h-3 bg-blue-900 rounded-full"
                style={{ width: `${course.progress}%` }}
              ></div>
            </div>
            <button className="mt-2 px-4 py-1 bg-blue-900 text-white text-sm rounded">
              Resume
            </button>
          </div>
        ))}
    ...
    <section className="w-full h-auto bg-white flex justify-center pt-2">
      <div className="w-[90%] h-auto rounded-md">
        ...
        <nav className="bg-blue-950 w-full h-[10vh] rounded-t-md text-white flex items-center justify-between px-8 pl-0 relative">
          <h3 className="bg-blue-950 text-center font-semibold rounded-tl-md py-5 px-8 text-white h-[10vh] w-[250px]">
            STUDENT PORTAL
          </h3>
          <ul
            className={`list-none flex gap-8 font-medium ${
              navOpen
                ? "flex-col absolute top-[70px] left-0 right-0 bg-blue-700 p-4 z-[1000]"
                : "hidden lg:flex"
            }`}
          >
          ...
          <div className="w-full lg:w-[75%] h-auto bg-transparent pt-8 pl-8">
            {selectedView === "Dashboard" && (
              <>
                ...
                <p className="text-lg ml-10 mb-6">
